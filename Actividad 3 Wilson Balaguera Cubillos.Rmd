...
dat_apart=subset(datos,datos$tipo=="Apartamento")
dat_casa=subset(datos,datos$tipo=="Casa")
```
...
#  <b style= 'color:#aa0000' >Modelo 1 - Simulaciónes </b>
...
```{r}
p=predict(regresion,newdata =list(areaconst=110))
...
```{r}
pl=exp(predict(regresionlog,newdata =list(areaconst=110)))
...
```{r}
plarea=exp(predict(regresionlogarea,newdata =list(areaconst=110)))
...
```{r}
pl2=exp(predict(regresionlog2,newdata =list(areaconst=110)))
